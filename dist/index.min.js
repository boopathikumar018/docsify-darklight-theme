const plugin = (hook, vm) => {
  var activeTheme;

  try {
    activeTheme = vm.config.darklightTheme.defaultTheme;
  } catch {
    activeTheme = 'light';
  }

  var defaultConfig = {
    dark: {
      background: '#091a28',
      textColor: '#b4b4b4',
      codeTextColor : '#ffffff',
      codeBackgroudColor : '#0e2233',
      borderColor : '#0d2538',
      themeIcon : 'url("data:image/svg+xml;base64,")'
    },
    light: {
      background: '#ffffff',
      textColor: '#34495e',
      codeTextColor : '#525252',
      codeBackgroudColor : '#f8f8f8',
      borderColor : 'rgba(0, 0, 0, 0.07)',
      themeIcon : 'url("data:image/svg+xml;base64,")'
    }
  };

  let themeConfig = { ...vm.config.darklightTheme , ...defaultConfig };

  var setTheme = (theme) => {

    activeTheme = theme;
    localStorage.setItem('DARK_LIGHT', theme);

    if(theme == "light") {
      for (var [key, value] of Object.entries(themeConfig.light))
        document.documentElement.style.setProperty('--'+key , value);
    } else if ( theme == "dark") {
      for (var [key, value] of Object.entries(themeConfig.dark))
        document.documentElement.style.setProperty('--'+key , value);
    }

    };
  
    hook.afterEach(function(html, next) {
      var darkEl = ` <div id="dark_mode"><p>.<p></div>`;
      html = `${darkEl}${html}`;
      next(html);
    });
  
    hook.doneEach(function() {
      if (localStorage.getItem('DARK_LIGHT')) {
        activeTheme = localStorage.getItem('DARK_LIGHT');
        setTheme(activeTheme);
      } else {
        (vm.config.darklightTheme.defaultTheme != undefined) ? setTheme(vm.config.darklightTheme.defaultTheme) : setTheme('light');
      }

      document.getElementById('dark_mode').addEventListener('click', function() { activeTheme === 'light' ? setTheme('dark') : setTheme('light');});

    });
  };
  
  window.$docsify.plugins = [].concat(plugin, window.$docsify.plugins);
